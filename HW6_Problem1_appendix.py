...
print(f'Ground-state energy is {E1[0]}')
...
def corr(A, B, v):
    return np.real(((v.T.conjugate()).dot((A.dot(B)).dot(v)))[0, 0])
Corr_1, Corr_2, Corr_3=[], [], []
for i in range(L+1):
    Corr_1.append(corr(N_tot[0], N_tot[np.mod(i, L)], psi_1))
...
#print(Corr_3)
plt.figure()
plt.plot(X, Corr_1, 'o--', label='correlation function for V1= 1')
